...
=== FIN DEL REGISTRO ==="""
        ...
        try:
            ...
            if temperatura or humedad or precipitacion:
                condicion = CondicionMeteorologica.query.filter_by(
                    lote_id=bitacora.lote_id,
                    fecha=bitacora.fecha
                ).first()
                if condicion:
                    condicion.temperatura      = float(temperatura) if temperatura else None
                    condicion.humedad          = float(humedad) if humedad else None
                    condicion.precipitacion_mm = float(precipitacion) if precipitacion else None
                    condicion.observaciones    = observaciones if observaciones else None
                else:
                    condicion = CondicionMeteorologica(
                        lote_id          = bitacora.lote_id,
                        fecha            = bitacora.fecha,
